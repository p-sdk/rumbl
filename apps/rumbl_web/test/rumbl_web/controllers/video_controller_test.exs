defmodule RumblWeb.VideoControllerTest do
  ...
  describe "with a logged-in user" do
    ...
    @tag login_as: "max"
    test "renders new video form", %{conn: conn} do
    ...
    @tag login_as: "max"
    test "updates user video and redirects", %{conn: conn, user: user} do
      video = video_fixture(user, title: "funny cats")
    ...
    @tag login_as: "max"
    test "does not update the video, renders errors when invalid", %{conn: conn, user: user} do
      video = video_fixture(user, title: "funny cats")
      conn = put(conn, Routes.video_path(conn, :update, video), video: @invalid_attrs)
      assert html_response(conn, 200) =~ "check the errors"
      assert Multimedia.get_video!(video.id).title == video.title
    end
